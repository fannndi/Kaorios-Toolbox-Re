...
- **Play Integrity API v1** (`https://playintegrity.googleapis.com/`, discovery returns 200) exposes `v1.decodeIntegrityToken`, `v1.decodePcIntegrityToken` and `deviceRecall.write`. All three need a Google Cloud project + OAuth, so the hook cannot call them — the raw token is encrypted and only Google's server can open it. But once you decode a token on your own project, `--decode-verdict` reads the decrypted verdict JSON and answers "did my spoof actually pass?" (`VerdictParser` in `:patcher`, no device needed). `deviceRecall.write` is a newer device-scoped signal worth tracking, as it may become another thing a spoofed device has to satisfy.
...
| `VerdictParserTest` | the decrypted `decodeIntegrityToken` verdict: STRONG implies DEVICE/BASIC, DEVICE without STRONG and BASIC-only are distinguished, an empty verdict list reads as unevaluated (never BASIC), missing sections still parse, and malformed input yields null instead of throwing |
