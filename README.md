...
- **Play Integrity API v1** (`https://playintegrity.googleapis.com/`, discovery returns 200, currently at revision `20260915`) exposes `v1.decodeIntegrityToken`, `v1.decodePcIntegrityToken` and `deviceRecall.write`. All three need a Google Cloud project + OAuth, so the hook cannot call them — the raw token is encrypted and only Google's server can open it. But once you decode a token on your own project, `--decode-verdict` reads the decrypted verdict JSON and answers "did my spoof actually pass?" (`VerdictParser` in `:patcher`, no device needed — it follows the official discovery schema, including the newer `environmentDetails`, `deviceAttributes`, `recentDeviceActivity` and `testingDetails` signals). `deviceRecall.write` is restricted to an early-access program for select Play partners, so it is not generally usable; still worth tracking as another device-scoped signal a spoofed device may one day have to satisfy.
...
- `Farewell-Patch-<profile>-<stamp>.zip` — patched jars, patched `build.prop`s (`system`, `product`, `vendor`, `vendor/odm/etc`), `etc/farewell/props.conf`, privapp permissions, patch marker. Mounts **only the partitions used by the manifest** (surya MIUI12: system/product/vendor; MIUI13/14 add system_ext; no `mi_ext`), TWRP mounts first with a deep-remount helper as fallback. **Flash-time backup**: every replaced file is copied to `/data/media/0/Farewell/backup-<stamp>/` (mirrored paths) with a generated `restore.sh`; `0:0 0644` per file; wipes package cache/dalvik and boot artifacts; **aborts without touching anything** if an entry cannot be written.
  - The deep-remount helper reads each partition's **filesystem magic** first: only `ext4` gets `e2fsck -E unshare_blocks` + `resize2fs`; a missing partition (MIUI 12 has no `system_ext`) is skipped instead of feeding `e2fsck` a nonexistent device. **EROFS and F2FS cannot be remounted read/write in place at all** — the image must be converted and the `super` partition rebuilt, which is exactly what [SystemRW/SuperRW "Make RW" (lebigmac's sysrw)](http://www.systemrw.com) does from recovery. On MIUI 13/14 (Android 12) `vendor` is often EROFS, so the flash prints which partition needs that step and aborts before changing anything; run MakeRW once, then flash the patch zip.
  - **Every write is verified and rolled back on mismatch.** After the copy loop each entry's on-disk size is compared against its zip entry; on any mismatch the flash restores backed-up files, removes files that did not exist before, and exits non-zero — a half-written boot jar is worse than a failed flash.
  - **Flash report**: every flash writes `/data/media/0/Farewell/flash-<stamp>.log` (profile, backup dir, file count, verify result, privapp-guard action, mount notes, entry list) so "what did the flash actually do?" is answered from one file instead of a second device session.
...
| `KeyboxRevocationRefreshTest` | The background-refresh lifecycle that used to need a device: `refresh()` fetches the status list on a background thread, parses it, and writes it to disk — and must stay best-effort. Drives it without the network via the injected `StatusFetcher` seam: a revoked source makes the synchronous check flip true and writes the cache file; a source that throws is swallowed so the chain is **not** flagged revoked and no cache is written. Pins the "a failed fetch must never disable the user's setup" contract |
